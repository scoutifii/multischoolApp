    ...
    * ********************************************************** */
    public function index() {
        ...
        if ($_POST) {
            ...
            if($exam_id){
                $this->data['exam'] = $this->result->get_single('exams', array('id'=>$exam_id, 'status' => 1, 'school_id'=>$school_id, 'academic_year_id'=> $school->academic_year_id, 'term_id'=>$term_id));
            }else{
                $this->data['exams'] = $this->result->get_list('exams', array('status' => 1, 'school_id'=>$school_id, 'term_id'=>$term_id), '', '', '', 'id', 'ASC');
            }
            ...
            create_log('Has been processed exam final result for class: '. $class->name);
        }
    ...
    * ********************************************************** */
    public function add() {
        ...
        if ($_POST) {
            ...
            $class = $this->result->get_single('classes', array('id'=>$class_id));
            create_log('Has been processed and saved exam final result for class: '. $class->name);
        }
