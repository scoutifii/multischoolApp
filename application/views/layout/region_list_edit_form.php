...
<div class="item form-group">
    <label class="control-label" for="region_id"><?php echo $this->lang->line('region'); ?> <?php echo $this->lang->line('name'); ?><span class="required" >*</span></label>
        ...
        <select  class="form-control fn_region_id" name="region_id" id="edit_region_id" required="required" style="width: 100%;">
            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>
            <?php $regions = get_region_list(); ?>
            <?php foreach($regions as $obj){ ?>
                <option value="<?php echo $obj->id; ?>" <?php if($school->region_id == $obj->id ) { echo 'selected="selected"'; } ?>><?php echo $obj->region_name; ?></option>
            <?php } ?>
        </select>
    </div>
